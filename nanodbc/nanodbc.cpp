...
std::list<datasource> list_datasources()
{
    ...
    do
    {
        ...
        if (rc == SQL_SUCCESS)
        {
            ...
            datasource dsn;
            dsn.name = string(reinterpret_cast<char*>(&name[0]), size(name));
            dsn.driver =
                string(reinterpret_cast<char*>(&driver[0]), size(driver));
...
std::list<driver> list_drivers()
{
    ...
    do
    {
        ...
        if (rc == SQL_SUCCESS)
        {
            ...
            driver drv;
            drv.name = string(reinterpret_cast<char*>(&descr[0]), size(descr));
